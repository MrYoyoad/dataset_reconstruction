...
def slide_r1_knobs(prs):
    ...
    H.add_tag(s, "part 4 · experiments and meaning", C.SL_W - C.MX - Inches(3.6), C.MY + Inches(0.15), w=Inches(3.6), color=C.GRAY)
...
def slide_r3_g0(prs):
    ...
    H.add_lead(s, "rank which images will leak from θ₀ and the image alone — n=24, indeterminate by pre-registration")
    ...
    H.add_eq(s, eq, rx, _Y0 + Inches(0.3), w=rw - Inches(0.2))
    _side_label(s, "the work the public model still has to do on the image", rx, _Y0 + Inches(1.75), rw,
                size=C.SZ_SMALL)
    H.add_card(s, rx, _Y0 + Inches(2.5), rw, Inches(1.75), "mechanism  (your KKT question)",
...
def slide_r5_hgate(prs):
    ...
    eq = render_math(r"\mathrm{mem}_i=\mathbb{E}_{\mathrm{seeds}}[\mathrm{margin}(D)-\mathrm{margin}(D\setminus\{x_i\})]",
                     "eq_r5_mem")
    H.add_eq(s, eq, rx, _Y0 + Inches(0.3), w=rw)
    ...
    H.add_card(s, rx, _Y0 + Inches(2.4), rw, Inches(1.8), "why this is the real check  (rigor gate)",
