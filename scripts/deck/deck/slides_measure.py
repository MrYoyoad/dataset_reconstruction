...
CAVEATS: The right panel's bars are the K=100 read (job 130198). The 3-way fix lowered the absolute scale but real-data d^2 still grows with K (arm B: 8 -> 22 as K 50 -> 100, ~46 at K=200; denominator flat) while the null stays ~0 at K=50/100/200 (job 212413) -- so quote the bars as a fixed-K relative statistic + p-value, never as a converged absolute. Because the null stays at ~0 and flat, that growth is benign signal-direction RESOLUTION, so d^2 is a consistent LOWER BOUND that tightens with K: quote magnitudes only at a fixed K, compare only at fixed K and fixed p; the detection p-value is the headline. Single-draw K-flatness is a knife-edge, hence the gate is a population mean over datasets, not one run.
...
_ROWS = [
    ...
    ("g₀ — who leaks", "is it predictable from the base model?", "base gradient predicts",  "⚠", "strong at low g₀; n=24 indeterminate"),
