...
ax.plot(Nl,cert,color=blue,lw=2.2,label="quotient boundary  k = r − N   (exact — Cor. 1)")
ax.plot(Nl,inv,color=orange,lw=2.4,ls="--",label="capacity boundary  k = m + r − N   (counting + simplex constraint; measured)")
...
ax.text(6.1,17.5,"+ m  per image\n(cap on the release's\ndegrees of freedom)",fontsize=10.5,va="center",ha="right")
ax.set_xlim(1,15); ax.set_ylim(0,46)
ax.set_xlabel("N  (private images / distinct representations)",fontsize=13)
ax.set_ylabel("k  (manifold dimension = unknowns per image)",fontsize=13)
ax.set_title("Two channels, two boundaries\nthe exact channel's capacity  k < m + r − N  is measured and sharp to one unit of k",fontsize=13,fontweight="bold")
