...
def main():
    ...
    print(f"# conv_encoder_ranklaw spec={a.spec} ckpt={a.ckpt} modules={[g['kind'] for g in geo]} N={a.N} r={a.r} "
          f"ks={a.ks} first={a.first} maxL={a.maxL} T_arm={a.T_arm} lr={a.lr} label={a.label!r} "
    ...
    npix = X_real.shape[0]
    print(f"# private join-key indices: {idx.tolist()} labels {y.tolist()}", flush=True)
    ...
    for l in range(D):
        ...
        emit(dict(part="PATCH_SPAN", layer=l + 1, kind=geo[l]["kind"], p_l=geo[l]["p_l"], P_l=geo[l]["P"],
    ...
    if a.T_arm:
        gchk = torch.Generator().manual_seed(a.seed + 99)
        rchk = min(a.r)
    ...
    for r in a.r:
        for first in a.first:
            ...
            A0 = {l: (sig[l] * torch.randn(r, geo[l]["p_l"], generator=gA)).to(dev) for l in adapted}
            arms = {}
            ...
            CONF[(r, first)] = dict(adapted=adapted, arms=arms)
            ...
            for (r, first), conf in CONF.items():
                ...
                for arm_name, arm in conf["arms"].items():
                    ...
                    for L in range(1, len(adapted) + 1):
                        parts = [blocks[l] for l in adapted[:L] if l in blocks]
                        if not parts:
                            st["rows"][L].append(dict(sv=None, n_rows=0)); continue
                        S = torch.cat(parts, 0)
                        st["rows"][L].append(dict(sv=torch.linalg.svdvals(S).cpu(), n_rows=int(S.shape[0])))
                    del blocks
            del Jl
            if dev.type == "cuda": torch.cuda.empty_cache()
        d_layer = [med(d_img[l]) for l in range(D)]
        print(f"  [{chart_label} k={k_eff}] d_j = rank M_l per module = {d_layer} (per image {d_img}) chart_err={chart_err:.2e} "
        ...
        for (r, first), conf in CONF.items():
            ...
            for arm_name, arm in conf["arms"].items():
                ...
                for L in range(1, len(adapted) + 1):
                    ...
                    rows, gaps, spec0 = [], [], None
                    for i, o in enumerate(st["rows"][L]):
                        if o["sv"] is None:
                            rows.append({**{lab: 0 for lab in LADDER}, "sigma_max": 0.0, "n_rows": 0}); continue
                        sv = o["sv"]
                    ...
                    emit(dict(part="RANKLAW", arm=arm_name, T=arm["T"],
                              config=dict(model=a.ckpt.split("/")[-1], spec=a.spec, chart=chart_label, k=k_eff, in_dim=in_dim,
                                          first_adapted=first, seed=a.seed, r=r, N=a.N, depth=D, lr=a.lr,
                    ...
                    print(f"  [{chart_label} k={k_eff} r={r} first={first} {arm_name} L={L}] modules={[l+1 for l in layers]} "
                          ...
                          f"gap={gap_at_corr if gap_at_corr is None else round(gap_at_corr, 2)}", flush=True)
        del acc
        print(f"  [{chart_label} k={k_eff}] done in {time.time()-tk:.0f}s", flush=True)
