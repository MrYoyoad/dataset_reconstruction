#!/usr/bin/env python3
"""ONE cell of the oracle-chart ladder, on the two examples the meeting document uses.
...
and so the convolutional example, at roughly 13 s per start, can run its charts in parallel rather than in series.
  ...
  privates   RAW photographs, not their chart projections -- so the release is fine-tuned on the photographs and the
  ...
  charts     oracle:<eps>  the ladder construction, VERBATIM from experiments/cifar/cifar_charts.py: perturb each
                           private photograph by relative noise eps, span those perturbed vectors, fill the rest of
...
eps is NOT the same quantity as the chart's projection error on the true photographs: eps perturbs the images before
the span is taken, so the true photographs do not lie in the resulting chart and their projection error is measured
separately. Both are reported and the figures are drawn against the MEASURED error.
  ...
  python -m experiments.oracle_ladder.ladder_cell --example mlp_motorcycle --chart oracle:0 --wrong_release
"""
...
EXAMPLES = {
    "mlp_motorcycle": dict(arch="mlp", cls="motorcycle", ckpt="models/exact_inversion/cifar10_mlp_newclass.pth", gate=(0.50, 0.90), epochs=60),
    "cnn_keyboard":   dict(arch="cnn", cls="keyboard",   ckpt="models/exact_inversion/cifar10_cnn_newclass.pth", gate=(0.80, 0.90), epochs=40),
}
...
def ssim(a, b):
    ...
    return float(km.ssim(a.reshape(1, 3, 32, 32).clamp(0, 1).float(), b.reshape(1, 3, 32, 32).clamp(0, 1).float(), window_size=3).mean())
...
def main():
    ...
    ap.add_argument("--data-root", default="data"); ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    ...
    ex = EXAMPLES[a.example]
    is_oracle = a.chart.startswith("oracle")
    eps = float(a.chart.split(":", 1)[1]) if is_oracle else float("nan")

    net, te, tr = train_backbone(ex["ckpt"], a.data_root, dev, ex["epochs"], ex["gate"][0], ex["gate"][1], ex["arch"])
    net = net.double()
    ...
    X_raw = Pri[perm[: a.N]].T.contiguous()                                        # the SAME eight photographs as the existing cells
    ...
    log(f"# {a.example} / {a.chart}{' / WRONG-RELEASE CONTROL' if a.wrong_release else ''}  backbone {te*100:.1f}% test"
        f"  |  eps={eps}  measured projection error of the true photographs: mean {float(proj_err.mean()):.4f}, "
        f"range {float(proj_err.min()):.4f}–{float(proj_err.max()):.4f}")
    if is_oracle: log("# NOT ATTACKER-AVAILABLE: this chart is built from the private photographs themselves.")

    # ---- the release, fine-tuned on the RAW photographs
    X_train = X_other if a.wrong_release else X_raw
    H = phi(X_train); y = torch.full((a.N,), m - 1, device=dev)
    ...
    C = A_T - Q @ (Q.T @ A_T)
    with torch.no_grad():
        Hr = phi(X_raw)                                                            # scored against the PHOTOGRAPHS always
    ...
    log(f"# release: rank B_T {Np}, rank C {int(torch.linalg.matrix_rank(C, rtol=1e-10))}, certificate residual at the photographs "
    ...
    for s in range(a.starts):
        ...
        if (s + 1) % 100 == 0: log(f"   {s+1}/{a.starts} starts, {time.time()-t0:.0f}s, landed {sum(r['landed'] for r in runs)}")
    ...
    for i in range(a.N):
        ...
        per.append(dict(i=i, best_err=float(err[bi, i]), landed=bool(err[bi, i] < 1e-2), best_objective=runs[bi]["objective"],
                        ssim_attack=ssim(xb, X_raw[:, i]), ssim_ceiling=ssim(X_proj[:, i], X_raw[:, i]),
                        ssim_control=max(ssim(X_found[:, s], ctrl[:, i]) for s in range(0, a.starts, max(1, a.starts // 80))),
                        proj_err=float(proj_err[i])))
    mean_ = lambda v: sum(v) / len(v)
    row = dict(part="oracle_ladder", example=a.example, arch=ex["arch"], class_name=cname, chart=("pca" if not is_oracle else "oracle"),
               eps=(None if not is_oracle else eps), attacker_available=(not is_oracle), wrong_release=a.wrong_release,
               N=a.N, r=a.r, k=a.k, T=a.T, lr=a.lr, seed=a.seed, starts=a.starts, backbone_test_acc=te, backbone_train_acc=tr,
               n_prime=Np, rank_C=int(torch.linalg.matrix_rank(C, rtol=1e-10)),
