...
def main():
    ...
    perm = torch.randperm(Xte_t.shape[0], generator=torch.Generator().manual_seed(a.seed)).tolist()
    ...
    for pool in a.pools:
        ...
        for k in a.ks:
            ...
            for _ in range(a.starts):
                ...
                errs.append(min(float(torch.linalg.norm(xh - X_real[:, i]) / torch.linalg.norm(X_real[:, i]))
                                for i in rec))                       # scored against the TRUTH, never the release
            se = sorted(errs)
            emit(dict(part="CHART_MISMATCH", pool=pool, k=k, N=a.N, r=a.r, rank_B_T=Np, cert_line=a.r - Np,
                      ...
                      frac_landed_1e1=float(sum(1 for e in errs if e < 1e-1) / len(errs)),
            ...
            print(f"  [{pool:7s}] k={k:3d}  chart err {float(chart_err.median()):.3f}  expl {chart.explained:.3f}  "
                  f"N'={Np}  landed<1e-2 {sum(1 for e in errs if e < 1e-2)}/{a.starts}  "
                  f"<1e-1 {sum(1 for e in errs if e < 1e-1)}/{a.starts}  median err {se[len(se)//2]:.3e}",
                  flush=True)
