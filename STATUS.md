...
>
> **Decisive robust cell (760909, RESULTS Step 25):** a NEW CLASS (EMNIST 'a' as an eleventh output, zero head row) at r = 64, k = 32 — release norm 1.0, adapter moves the logits by 43%, margins −9…−3 → +6…+14 — **all eight letters recovered from random starts, no recipe, no labels (38% of 500 starts)**, certificate residuals 1e-12 at the truths with a moving adapter. Under fp32 TRAINING: **8 of 8 at k = 16** (82.6% of starts) and 6 of 8 at k = 32 at the noise-matched tolerance (tight-tolerance search running); half-precision training keeps the release's norm but degrades its directions 2–25% and the certificate search finds nothing (fp16: 0 of 8); random head row reproduces the zero row in every number; bf16 training moves the release 12–72% (accumulation), fp16 training zeroes any release whose update lr·R/N underflows (margin ≳ 10). Landing error ≈ 5 × certificate residual at the truth, in every cell.
