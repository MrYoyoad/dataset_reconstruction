...
> **Attacker's stopping rule (2026-09-04, jobs 85300/86888/87369/88743/89853):** the image finishes long before the residual does. At the instance-level chart the image error falls to 4.2% at residual 4.2e-3 and jumps to 7.5% by 2.8e-3 — a sharp knee, no flat bottom, so the attacker must tune the stop; "descend as far as the arithmetic allows" loses almost half the fidelity. The knee is per-format and rises with both the training roundoff and the chart's conditioning; at the coarser chart it lies below every floor and no such penalty exists. Above the knee three different releases in three arithmetics agree to 0.4% (fidelity is set by the residual, not the format); below it they diverge by 2× as each veers toward its own minimiser.
