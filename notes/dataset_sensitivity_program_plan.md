...
mean/std-matched ssim_norm — the metric that inflated the earlier retracted claim — reads 0.57–0.61 with
visible digits in the grids: the reconstructions are soft-but-recognizable, not garbage; the raw/mean-
baseline bar above remains the honest one.) The *geometry* leaks (r_J / q_eff are high, §I.4); the
*pixels* do not yet.
