...
**Correction (important):** the anchor is **Jang et al. 2024** (ICML) — LoRA needs **r ≳ √N** (rank on the order of √N, not the r ≳ N we previously cited) to remove spurious local minima; full-FT admits a rank-√N solution. **Our extrapolation (not Jang's stated bound):** a constraint-counting argument makes the threshold output-dimension-dependent — r(r+1)/2 > K·N, i.e. √(K·N) — which explains our anomaly. For N=10: binary (K=1, single BCE logit) needs r(r+1)/2 > 10, cleared by r=8/16/32 alike (binary q_eff flat); 10-class (K=10) needs r(r+1)/2 > 100, so r=8 (dof 36 < 100) sits below the threshold and r=16 (dof 136 > 100) above. The reversal gap collapsing exactly there is the K·N counting showing up in leakage space. (Jang is a landscape result — it anchors the boundary by analogy; the K-dependence and the informational claim are ours.)
...
Does the adapter betray what it was trained on (which content, how many, what balance), not just its recipe? LoRA has an exact gauge symmetry BA = (BR)(R⁻¹A): the raw factors carry the seed, only ΔW = BA is gauge-invariant.
