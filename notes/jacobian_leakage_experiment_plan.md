...
## Part 7 — Crosswalk: the supervisor's whitened-Jacobian refinement → what we actually ran (added 2026-08-30)

The refinement (2026-08-20, the message that upgraded raw `σ_min(J_full)` to `J_SNR = Σ_seed^{-1/2} J`,
`q_eff(ε)`, tangent-coordinate recovery, the `(N,r,L)` phase diagram, the disjoint-adapter control, the
whitened `v_min/v_max` figure, and the staged-Jacobian bridge decomposition) is the design basis of
Parts 1–5 above. This section records, element by element, **which experiment realized it, what came out,
and what is still open** — so the idea is never re-derived from scratch. Posture: observe, don't conclude;
every number bounds the WEAKEST (prior-free, per-image, adapter-only) attacker.

| Refinement element | Realized by | Result (observed) | Open |
|---|---|---|---|
| **Don't compare `σ_i(J)` to a scalar `τ_seed` — whiten by the anisotropic `Σ_seed`** | `jacobian_spectrum.py`: `estimate_sigma_seed` / `snr_spectrum` (Woodbury) / `q_eff` / `q_eff_colspace`; discrete-dataset twin `dataset_sensitivity/whitened_metric.py` (`d² = Δμᵀ Σ⁻¹ Δμ`, 3-way cross-fit + permutation null) | Units point demonstrated empirically in arm B: energy-SNR 0.25 says "masked", whitened permutation p = 0.008 says detectable (N=8). Full `Σ_seed` is UNMEASURABLE (`eff_rank ≈ S−1` at every S, dimY=14272) → the sound object is **`q_eff\|col(J)`**, a conservative LOWER bound (Schur). | Whitened MAGNITUDES are normalizer-fragile (arm-B λ-tail artifact, retracted); quote the rank-based p-value, or `q_eff\|col(J)` with `{K,2K}`/ρ ranges. |
| **`q_eff(ε) = #{i: ε σ_i(J_SNR) > 1}` as THE privacy quantity** | J1 (jobs 983139 → 983585/983882/983933); Round-B anchor; rank sweep (581629) | Anchor mnist gelu N=10 k=8 (Nk=80) T=1000 lr=0.5 r=8 S=320: **q_eff@ε1 = 59 (binary) / 36 (10-class)**, r_J=80. Rank sweep: gap 23→13→0 at r=8/16/32 (low-rank effect, gone at full-FT). | `q_eff` absolute values carry the Hotelling-class upward bias (whitened_sensitivity_metric.md M2) — differentials are robust, absolutes need the permutation estimator before being quoted as "N recoverable directions". |
| **Seed covariance from training randomness only** | J1 `Σ_seed` = Cov over LoRA-B0 init draws (full-batch GD ⇒ B0 is the only stochasticity) | B0-init noise lives in the B-block, data signal enters the A-block (`A₀=0` mechanism) ⇒ noise couples into col(J) through ≤1 mode (`iso_ratio` 0.01–0.10); init randomness is a WEAK, non-scaling defense. Robust across T=5/20/50. | **SGD / minibatch / data-order noise phase in the tangent testbed — NOT RUN** (the noise source that would actually live in col(J)). Only proxy: valley B2 gate (SGD-order noise, divergent, ~30% narrower). This is the single biggest gap to the refinement. |
| **Central law: `q_eff/q < 1 ⟺ reconstruction fails` (or degrades with `q_eff/q`)** | Not tested as a curve. Evidence pieces: H2 (147972), DI wall, three-worlds classification | H2: in the deterministic regime `hard_rank = Nk` everywhere (ill-conditioning, not rank deficiency) — the ceiling is purely NOISE-relative, so the law can only be tested in the noisy regime. `q_eff` high (up to 156/160) while direct inversion collapses N=4→10 (0.57→0.27) ⇒ classified World B (extraction-limited), not World A. | The `q_eff/q`-vs-recovery curve and the per-coordinate CRLB scatter (P1/P2) were never produced under a col(J)-living noise source. Depends on the SGD-noise phase above. |
| **Hide `Nk` numbers in orthonormal tangent directions `U_i`; recover `a` from `Y` (the "embarrassingly controlled" first experiment)** | J0 (job 982855): `build_tangents` (qr / svd-decay / pca / difference / pca_tail / residual), `forward_Y`, `exact_jacobian` (`jvp_double`), `recover_a` | AD validated (toy FD 5.9e-10, MNIST FD 3.9e-9); coordinates recoverable; `σ_i(J)` tracks the injected svd column decay. J0's "N=4 collapse" was T=5 UNDERFITTING (T-sweep: eff_rank 9.3→12.7 at T=5→50). | — (machinery done; GELU-only by necessity: `modified_relu` has no double-backward). |
| **`k` from the top-k well-conditioned directions of `J_g`, never a nominal latent width** | `_pca_basis` on a public bank (linear generator ⇒ constant Jacobian = PCA); H1 discriminative bases (151183) | H1: genuinely different INPUT subspaces (overlap 0.058) collapse into ~the SAME col(J) (0.95) — collinearity is intrinsic to `∂Y/∂x`, basis-invariant; the discriminative (truly private) subspace is tiny (rank ≤ N−1), most masked, least recoverable. | `k` is still a linear-PCA construction; a nonlinear generator's local `U_k(z*)` (the refinement's exact proposal) was not built. |
| **Multi-knob phase diagram `(N, r, L)`; fixed-epochs AND fixed-steps** | `r`: rank sweep 581629 (r∈{2,4,8,16,32}); `N`: J0/J1 N∈{2,4,10}, arm B N∈{2..64} (whitened detection flat in N); width/`L`: not swept | Reconstruction boundary vs spectral boundary NOT cross-checked (no recon endpoint in the same cells). Full-batch GD ⇒ fixed-steps ≡ fixed-epochs, so the dual-sweep hygiene is moot until minibatch is introduced. | **`L` (number of adapted modules) and H5 width never varied** — single module `target_layers=(0,)` throughout. The joint collapse surface (P3) is unrun. |
| **Disjoint-adapter control `Δ_adapter = Q(X̂(Y_A),X_A) − Q(X̂(Y_B),X_A)`** (J3, World-C test) | Not run as a reconstruction control (the adapter-only pixel endpoint is dropped as a failed experiment — see memory). Closest realized analogs: arm-B reseed-vs-reseed null (flat ~0 at every N); atlas/instance zoo closed-set identification (composition +0.989; instance-level LOIO kNN 1.000, chance 0.125); eco GATE-3 disjoint-content overlap = 0; crux `ctrl_margin_norm` (same-class control image, not an unrelated adapter). | The matched `X_B → Y_B` control must be attached to whatever adapter-only inverter is built next (E7) — it is the only thing that separates leakage from prior hallucination. |
| **Whitened `v_min`/`v_max` figure** (J4) | Not run. Nearest: `figures/rank_sweep/rank_sweep_spectrum_r8.png` (σ(J_SNR) geometry), valley arm G `J·a_nn / J·a_far` (full≈LoRA), arm F g₀ footprint | — | Render `v_min`/`v_max` of `J_SNR` via `U_k`, retrain, compare to the noise ellipsoid. Cheap once a col(J)-living noise exists. |
| **Staged Jacobians `J_G → J_Y → J_{D∘Y}` + DPI bridge diagnosis** (J5) | Not run. GB decoder exists (0.951 cosine on m=8 batch gradients, job 956994); activation crux measured end-to-end only (free-c ladder 392821: kinked leak ~0.47 vs smooth ~0.09) | P5 (self-gated collapse at the gradient stage) and P6 (decode vs hallucinate) untested. | sci-summary §7 item 3 "J-composed Fisher bridge" — the decoder-vs-information-floor gap. |
| **Order: 2–4 images, 4–16 coords, one module, deterministic → seeds+whiten → N → r → modules → activations** | Followed through "seeds+whiten" and `r`; `N` partially; modules/activations-in-the-Jacobian not reached | — | Resume at: SGD-noise phase → `L`/width → staged Jacobians. |

**One-line status of the refinement.** The measurement system it asked for exists and is validated (J0/J1,
`q_eff|col(J)`, the whitened d² with permutation null); its two headline numbers so far are the 59/36
reversal and its closure with rank. What has NOT been done is exactly the part that makes it a *law*:
a noise source that lives in col(J) (SGD/minibatch), and the reconstruction-vs-`q_eff/q` curve with the
disjoint-adapter control attached. Everything downstream (J3–J5) waits on that.
