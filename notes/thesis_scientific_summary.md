...
We ask how much a released fine‑tuned adapter leaks about its private training set, and whether that leakage can be turned back into the images. Two observations so far (held loosely): **(1) the leakage GEOMETRY looks real and high** — an attacker can recover a large fraction of the private‑data directions from the adapter, and, per image, *how much* an image leaks appears predictable from the public base model alone (its base‑gradient‑norm g₀); **(2) the information needed to reconstruct IS present** — with the full model gradient we already recover recognizable images; a robust adapter‑only inversion is the immediate next step (an extraction problem, not a missing‑information wall). A counter‑intuitive sub‑result: **multi‑class training does not amplify leakage — it leaks *fewer* recoverable directions** than binary, an effect that lives at low LoRA rank and vanishes as the adapter approaches full fine‑tuning. The active frontier extends this to the full‑weight regime (the "valley‑width" comparison) and to a per‑image privacy predictor.
...
![Identifiability: multi-class (orange) leaks fewer recoverable directions than binary (blue) at every attacker budget ε — the leakage GEOMETRY is real and high.](figures/combined/leakage_identifiability_plus_reconstruction.png)

### 2.2 The reversal is a LOW‑RANK effect that vanishes at full fine‑tuning

Sweeping LoRA rank r ∈ {2,4,8,16,32} (job 581629): the multi‑class "leaks‑fewer" gap **closes monotonically 23 → 13 → 0** across the converged ranks r=8/16/32. Binary q_eff is flat (~58); the 10‑class value climbs to meet it as r → N and beyond (Jang 2024 proves r ≳ √N — NOT r ≳ N; the multi‑class generalization by constraint‑counting is r ≳ √(K·N), ≈√100≈10≈N for 10‑class — the K‑factor is OUR extrapolation, Jang's stated bound is the scalar √N). So the effect is genuinely *low‑rank‑specific*, not a general property of cross‑entropy; and the noise‑coupling mechanism that explains it at r=8 *decouples* by r=16. Low‑rank measurability bracket: r=1 is the only true capacity floor (never memorizes, job 635386); r=2/4 are underfit at the matched recipe and quarantined.
...
### 2.3 Reconstruction: the information is present (positive framing)

With the full model gradient we recover **recognizable** training images across datasets, image‑counts, and activations (mnist/cifar/flowers near‑exact; fashion partial) — the information IS in the weights and IS recoverable. Turning this into a robust **adapter‑only** inversion (a better decoder / prior‑equipped attack) is the immediate next‑weeks work; it is an *extraction* problem, not a missing‑information wall (§2.5 shows full fine‑tuning holds even more signal to work with). *(The earlier adapter‑only mean‑baseline read came from an under‑cooked inversion setup and is not treated as a result.)*
...
**Honest caveat (do not overstate):** at scale the predictor is weaker — ρ = +0.777 (n=24, job 272504), graded **INDETERMINATE** (bootstrap CI half‑width 0.189 > the 0.15 target). The relationship **SATURATES with g₀**: tercile ρ = **+0.881 (low‑g₀) / +0.500 (mid) / −0.119 (high‑g₀)** (verified from source, ascending‑g₀ order) — g₀ predicts strongly where the base model does *little* work (low g₀) and FLATTENS/slightly‑reverses once g₀ is already large; *why* it saturates is OPEN. (Not "strong where leakage is high" — that reading was backwards.) And there is a live **counterexample**: injected out‑of‑distribution USPS digits have higher g₀ yet leak *less* (n=2 — likely a smoothness / high‑frequency‑energy confound, to be arbitrated at scale).
...
Two links in the chain behave oppositely: **smoothness → linearization fidelity HOLDS** (softplus is the best linearizer; the softplus_b(β) knob is monotonic ~12× over β, job 911475), but **linearization → leakage magnitude is REFUTED** (Spearman ≈ +0.03 on MNIST; it even inverts on flowers). The 152‑config first‑pass (job 389926) and now the **realistic free‑coefficient wc‑ladder (job 392821, LANDED)** agree: there is **no clean monotonic smoothness law — it is a two‑cluster effect**, with **kinked relu / leaky_relu / selu leaking most** (ctrl_margin_norm kinked‑mean ~0.46 vs smooth‑mean ~0.09). Crucially the free‑c result **CONFIRMS this and does NOT flip the oracle ranking on MNIST** — The ROBUST, ordering‑independent facts: kinked relu/leaky_relu/selu leak most (a **two‑cluster gap**, kinked‑mean ~0.47 vs smooth‑mean ~0.09 on ctrl_margin_norm — needs no ranking), and Spearman(smoothness, leakage) is NEGATIVE at every rung and strengthens (more negative) with weight‑change (≈−0.2 → −0.8). The EXACT Spearman is smoothness‑ORDERING‑dependent — there is no unique total order of "smoothness" over 13 activations (that non‑uniqueness IS the two‑cluster / not‑monotonic point) — so we quote the sign+trend+cluster‑gap, not a single canonical Spearman. *(Honest caveats: two‑cluster, not a monotonic law — selu clusters with the sharp kinks and the within‑smooth trend differs; MNIST only, N=2, T=1, n=13 activations — exploratory; the flowers band, where the oracle↔free‑c dataset‑dependence would show, is still pending; feature‑stability‑vs‑T (job 390026) still running.)*
...
- **Gradient‑bridge decoder** reaches **0.951 cosine** to the true gradient (job 956994) and leaks end‑to‑end for shallow nets with abundant proxy data — the robust adapter‑only pixel inversion is the open next‑weeks extraction problem (§2.3) — it degrades with input dimension, depth, and N>2.
...
| Supervisor top ask: activation × anchor × linearization crux | **free‑c CONFIRMED (MNIST)** | free‑c wc‑ladder LANDED (392821): kinked relu/leaky/selu leak most, robust across the matched‑wc ladder (kinked‑mean ~0.47 vs smooth ~0.09; Spearman(smoothness,leakage) NEGATIVE ≈−0.2→−0.8, exact value smoothness‑ordering‑dependent), does NOT flip the oracle — a real realistic‑attack result. Two‑cluster not monotonic; n=13, exploratory. Flowers band + feature‑stability‑vs‑T (390026) still running |
...
| Gradient bridge decoder | **Phases 1–2 DONE** | 0.951 cosine (956994); robust adapter‑only pixel inversion = next‑weeks work (§2.3) |

---

## 6. Honest limitations & open loops (the things to close)

1. **Robust adapter‑only inversion — the open extraction problem.** The information is present (§2.3 full‑gradient recoveries; §2.5 more signal in full FT); the next step is a better / prior‑equipped decoder — optionally bounded by a J‑composed Fisher floor to say how far the decoder is from that information.
2. **The activation crux (supervisor's top ask) — free‑c CONFIRMED on MNIST.** The realistic free‑coefficient wc‑ladder (job 392821) LANDED and confirms kinked relu/leaky/selu leak most, robust across the matched‑wc ladder (two‑cluster gap ~0.47 vs ~0.09; Spearman(smoothness,leakage) NEGATIVE, strengthening with wc; exact value ordering‑dependent), NOT flipped from the oracle. Remaining: the flowers band (dataset‑dependence check) + feature‑stability‑vs‑T (390026 running). (The rest of the program remains GELU‑only.)
...
3. **Build the J‑composed Fisher bridge** — quantify how far the decoder is from the information floor (J‑composed Fisher)
