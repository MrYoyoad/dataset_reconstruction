...
Two links in the chain behave oppositely: **smoothness → linearization fidelity HOLDS** (softplus is the best linearizer; the softplus_b(β) knob is monotonic ~12× over β, job 911475), but **linearization → leakage magnitude is REFUTED** (Spearman ≈ +0.03 on MNIST; it even inverts on flowers). The 152‑config first‑pass (job 389926) and now the **realistic free‑coefficient wc‑ladder (job 392821, LANDED)** agree: there is **no clean monotonic smoothness law — it is a two‑cluster effect**, with **kinked relu / leaky_relu / selu leaking most** (ctrl_margin_norm kinked‑mean ~0.46 vs smooth‑mean ~0.09). Crucially the free‑c result **CONFIRMS this and does NOT flip the oracle ranking on MNIST** — Spearman(smoothness, leakage) is positive and *strengthens* with weight‑change across the matched‑wc ladder (+0.24 at wc≈0.005 → +0.82 at wc≈0.3), kinked leading at every rung. *(Honest caveats: two‑cluster, not a monotonic law — selu clusters with the sharp kinks and the within‑smooth trend differs; MNIST only, N=2, T=1, n=13 activations — exploratory; the flowers band, where the oracle↔free‑c dataset‑dependence would show, is still pending; feature‑stability‑vs‑T (job 390026) still running.)*
...
| Supervisor top ask: activation × anchor × linearization crux | **free‑c CONFIRMED (MNIST)** | free‑c wc‑ladder LANDED (392821): kinked relu/leaky/selu leak most, robust across matched‑wc ladder (rho +0.24→+0.82), does NOT flip the oracle — a real realistic‑attack result. Two‑cluster not monotonic; n=13, exploratory. Flowers band + feature‑stability‑vs‑T (390026) still running |
...
2. **The activation crux (supervisor's top ask) — free‑c CONFIRMED on MNIST.** The realistic free‑coefficient wc‑ladder (job 392821) LANDED and confirms kinked relu/leaky/selu leak most, robust across the matched‑wc ladder (Spearman +0.24→+0.82), NOT flipped from the oracle. Remaining: the flowers band (dataset‑dependence check) + feature‑stability‑vs‑T (390026 running). (The rest of the program remains GELU‑only.)
