...
2. **LoRA in the NTK Regime**: When LoRA rank r is sufficiently large (r ≳ √N — Jang et al. 2024 proves rank on the order of √N to kill spurious minima, NOT r ≳ N; the output-dim √(K·N) refinement is our own constraint-counting extrapolation, not Jang's stated bound), LoRA optimization in the NTK regime converges to the same global minimum as full fine-tuning (Jang et al., 2024). This means LoRA weights BA encode the same support vector geometry as full weights — the KKT stationarity condition adapts to a coupled system where B^T(Σ λ_i y_i ∇_W Φ(θ; x_i)) = 0 and the analogous condition for A.
...
- **PEFTLeak / Gradient Inversion on PEFT (Sami et al., 2025)** — **CORRECTION (2026-08-21):** this is a **MALICIOUS-server analytical attack** (poisons the pretrained model + adapter modules so no two tokens/patches share neuron activations), NOT an honest-but-curious "PEFT focuses gradient info" result. It does **not** validate a *passive* direction; it is a malicious upper bound. Attacks generic adapters (needs a post-adapter activation). Code: `github.com/info-ucr/PEFTLeak`. [arXiv:2506.04453] `papers/Sami_et_al_2025_Gradient_Inversion_PEFT_CVPR2025.pdf`
- **MineGrad (Sami, Sen, Güler, AISTATS 2026)** — LoRA-specific successor to PEFTLeak. **Malicious server** embeds data in LoRA gradients via poisoned position encodings + identity attention + coordinate-selector `A` init; breaks the DAGER `tokens ≤ rank` bound by tiling `≈2(S−1)·r` disjoint token-slots across **multiple encoders/modules** (leakage scales with #independent modules, not `r`). Vision: ViT/CIFAR, LPIPS 0.20. [arXiv:2608.01521, code `info-ucr/MineGrad`] `papers/Sami_Sen_Guler_2026_MineGrad_LoRA_Gradient_Inversion_AISTATS.pdf`. **Full teardown + bridge re-plan: `notes/minegrad_analysis.md`.**
- **Yao 2024, Risks When Sharing LoRA Fine-Tuned Diffusion Weights** — **closest PASSIVE competitor** to the gradient bridge: learned map from final LoRA weights → private images, vision (diffusion, identity-level). No malicious modification. [arXiv:2409.08482] — **read before claiming passive-LoRA-vision novelty.**
