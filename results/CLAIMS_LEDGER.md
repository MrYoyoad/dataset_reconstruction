...
|---|---|---|
| **Does the attack recover anything under a PERCEPTUAL criterion, where the exact bar says no?** | **Q11** (job 356492) | **Yes — public PCA identifies 7/8 private letters in a 100-way line-up while landing 0/8 exactly.** Every "no chart works" claim in this file is a TIER-1 statement and must be labelled as one |
...
| Q11 | **The recovery criterion decides the headline.** Tier 2 (truth top-1 in a line-up of truth + 99 public decoys of the class, by SSIM and by base-model features) against tier 1 (relative pixel error < 1e-2), letters ladder, certificate vs the RAW truth: exact landings 7/8 at chart error 0.0069 → **0/8 from 0.0139**, while SSIM top-1 stays **8/8 to chart error 0.26** and is **7/8 on the attacker-available public PCA chart (0.312) where tier 1 reads 0/8** — and the chart's own projection is also 7/8 there, so the attack reaches the chart's ceiling. Motorcycle 8/8 to 0.6, 2/8 at PCA; keyboard 7/8 early → 0/8 at PCA. | 767 arms over every saved cell of the 18 Sept package (ladder, WP2, bootstrap, decoder), decoys from the public TRAIN split, fixed seed | tier 1 unchanged and still behind every certificate-floor claim | **attacker skill on ORACLE cells** — there the chart is spanned by perturbed privates, the projection is 8/8 by construction and the wrong-release control scores 4/8 on SSIM (feature rank separates: 1/8 vs 8/8); on `d15` cells SSIM is uninformative (control 6–7/8, feature 0/8); no human study — SSIM/feature rank is a proxy for "a person could tell" | 356492 (smoke 356475) | read-rows (perceptual-id lane) | HELD |
...
| Q5 | **A pretrained SD VAE cannot be a chart for 32×32 CIFAR on fidelity alone**: autoencoding ceiling on the ladder's eight motorcycles **0.055** at 8× (0.078 at 4×, 0.21 at 2×) against the gate bracket 0.0124–0.0186 → **CEILING-BOUND** (pre-registered). Attacker-anchored local latent chart 0.36 at k=16 (pixel PCA 0.34); oracle latent anchor 0.045; global latent PCA worse than pixel PCA. | `sd-vae-ft-mse` (diffusers 0.32.2), motorcycle set (ladder idx 23 32 89 56 41 26 34 61), k=16, K=64, 200 Adam steps, FP32 | exact replicate/block-mean resize (floor 0.0000); w=0 at the oracle anchor reproduces the ceiling exactly | any other decoder; the remaining local-chart rows (K/k grid, 356034–356098 running). **Interim, all three sets: ceiling 0.055 / 0.024 / 0.021 (motorcycle / keyboard / letters-a at 8×) vs brackets 0.0124–0.0186 / 0.0045–0.0090 / 0.0069–0.0139 → CEILING-BOUND on every set; global latent PCA < pixel PCA at every k; letters-a attacker-anchored local chart 0.170 at k=32 beats pixel PCA 0.235 — first chart to beat PCA here — at 12× the gate's high end.** **Final (18 cells, 72 rows, 0 solver failures): CEILING-BOUND on all three sets**; best attacker arm at `k ≤ 66` 0.2550 / 0.2101 / 0.1022. **Letters caveat: the oracle-anchor fit reaches 0.0092, INSIDE the 0.0069–0.0139 bracket — the decoder's latent family contains those privates, so the obstruction there is the anchor + the `k ≤ 66` cap, not the decoder** (CIFAR oracle floors 0.0371 / 0.0207 stay above their high ends). **Oracle locality buys nothing** (truth-neighbour ≈ proxy-neighbour everywhere); only the oracle latent does. | smoke 355910; full 356034–356116 | read-rows (WP3) | HELD |
...
## R. Multilayer parameter program — r sweep, architectures, layer subsets, drift (2026-09-18, plan `notes/plan_2026-09-18_multilayer_parameter_program.md`)

| id | text | measured on | holds under | NOT shown | jobs | register | status |
|---|---|---|---|---|---|---|---|
| R1 | **The two depth laws separate on a real CNN and the corrected law `min_j(d_j + Σ_{l<j} q_l)` holds as an exact integer with a real gap, 6/6 cells; T5.2 over-predicts 6/6.** Bottleneck CNN, r swept at every module: at r = 64 (first live module conv 4, 32 rows × 4 positions) and r = 128 (conv 3, 11–15 rows × 16) with k = 784 the ceiling binds and the laws differ by 80–210; measured stacked rank@1e-10 = 128/127/128 and 304/335/368 = the corrected prediction of the SAME seed's row (rank C_3 = 11/13/15 → q_3 = 176/208/240), `gap_at_corrected` 2e4–5e9, `rank_test_outcome = compare`. `cond_at_1e10` non-monotone in r: 10 → 50 → 3e7 → 4e7 → 5e2, and three orders across seeds at fixed r. | `mnist_conv_bottleneck.pth` (gate PASS), N = 8, three seeds (different truths), zero drift, prefix stacks, first ∈ {1, 3, 5} | r ∈ {8 … 256}, k ∈ {32, 128, 384, 784}; r = 8 is the all-vacuous control; r ≤ 32 dense-only, r = 256 conv-2-pinned (laws coincide) | the T arm at r = 64/128; any other net (P2(i) plain deep conv is vacuous at r = 256, job 366150); a solve; the d15 MLP's ~6% under-prediction (no-gap regime, §5b) is a different object and not contradicted | 366149 (P1-CNN), 366150 (P2i), smokes 366139/366143 | read-rows (coordinator, single read) | INTERIM — needs a second read |
| R2 | **A layer's certificate rank ladder in r is exactly `min(r, p_l) − N'_l` per module, and the live-module set per r is the pre-registered one** from `N'_l = [9, 232, 117, 32, 8, 8]`: conv 1 always vacuous, conv 2 live only at r = 256 (24), conv 3 from r = 128 (11–15) , conv 4 from r = 64 (32), dense/head from r = 16 (r − 8). | same | same | drift (T arm) | 366149 | read-rows | INTERIM |
| R3 | **With the layer below the adapted layer also adapted, the training span of the adapted layer's input grows by N per SGD step on a real net (`N' = 150 ≈ 8·(T−1)` at T = 20), so the full certificate at r = 64 is contaminated (`rank B_T = 51 < N'`, rank C = 13, residual 5e-7 off the floor) after ~7 steps regardless of lr or the lower rank; the truncated certificate carries `K_l` = 0.053 (twin) / 0.0089 (strong MLP); the zero-drift control sits at the FP64 floor (rho 4e-16).** | `mnist_mlp_d15w1000_full` target 2 with layer 1 adapted (r 16), `mnist_mlp_strong` target 2/3; lr 0.01, N = 8 | smoke: 20 starts, seed 1 | the Rule A / Rule B curves (P3 running: 366211–366225, both certificates, on-chart privates, 200 starts, 3 seeds); momentum/wd (P6, placeholders unfilled) | 366163, 366203 (re-smoke, identical) | read-rows (drift builder + coordinator) | INTERIM (smoke) |
| R4 | **A chart hand-over opens an exact null space: two different 32-charts span 64 > rank C = 56, and the union Jacobian `C·J_φ·[V_t V_{t+1}]` has rank exactly 56 with a 1e13 gap, in every round pair on both releases; the union is ×5–8 worse conditioned than either chart.** On CIFAR global charts 7–8/8 recoveries carry the alias flag (equal-scoring different chart points), so the coverage reading does not hold there; on the MNIST recovery-anchored chain it holds in round 1 (0/8) and weakens later (1–2/8); that chain's charts converge (min principal angle 7.6° → 1.1°). | 355987 (MNIST), 355988 (CIFAR global rounds) saved charts; reference `x*_chart` | analysis on saved artifacts, single seed | identifiability on the union (measurement only, audit 10c); CIFAR variant-B pairs | 366170, 366171 | read-rows (chart builder + coordinator) | INTERIM |
| R5 | **Chart conditioning grows ≈ ×4 per +16 chart coordinates for PCA, local PCA and AE alike (AE × its own decoder cond 13–34), while projection error improves with k — the two predictors of landing oppose across k; which one orders landing is UNDECIDED on this release** (two joinable landing cells, same order under both). | CIFAR CNN motorcycle head release, r = 64, k ∈ {16, 32, 48} | raw and on-chart privates | k = 32/48 landing cells on this release; SD-VAE (no saved latent chart ≤ 48) | 366191–366193 | read-rows | INTERIM |
| R6 | **On a ResNet-18 stage the conv certificate is empty at every deployed rank: with 64 positions per image the 8 images give 512 linearly independent base patches at every conv (`N'_l = N·P_l` exactly), so rank C = `min(r, p_l) − 512` = 0 for r ≤ 512; live only at r = 1024, where one conv pins any chart (rank = k, cond 10–23, no gap).** Residual at the truth 1e-15 everywhere (exactness holds; it is the rank that dies). | `cifar10_resnet18.pth` (gate PASS 99.806% / 8.04e-3), torchvision layer3, BN eval, ReLU, N = 8, seed 1 | r ∈ {64, 512, 1024}, k ∈ {32, 128}, L ≤ 2 (smoke) | N = 4 and layer4 (fewer positions) — full stage submitted; a T arm; a solve; other seeds | 366181 (train), 366250 (smoke) | read-rows (ResNet builder + coordinator) | INTERIM |
| R7 | **On the 15-layer MLP twin depth adds exactly `r − N` ranks per layer with a real gap (1e10–1e15) up to a conditioning wall ≈ 1e9–1e10, three seeds identical at every r ∈ {16 … 256}; beyond the wall — which is exactly where the two depth laws separate — there is no gap and the effective rank sits 1–6% under the corrected law and far under T5.2 (r = 108, first = 3, L = 8: 414/414/416 vs 424/417/420 vs 713; the k* = 417 cell of 365681 reproduced on the gate-PASS twin).** Conditioning grows ≈ ×3–10 per stacked layer and with r; the deployable `first = 3` arm is 1–2 orders worse than raw-input adaptation at every L. | `mnist_mlp_d15w1000_full` (gate PASS), N = 8, zero drift, k = 784 (k ≤ 128 pinned), prefix stacks | r ∈ {16, 32, 64, 108, 256}, L ≤ 8, three seeds | **r = 8 rows are DEAD** (relative-only floor at r − N = 0; LESSONS 2026-09-18); drift; a solve; a gapped cell that separates the laws on an MLP (none exists here; the CNN R1 supplies it) | 366148 | read-rows (coordinator, single read) | INTERIM |
| R8 | **Layer subsets (prefix, suffix, middle, alternating, random, single) on the bottleneck CNN: the stacked rank is set by the SHALLOWEST live module in the chosen set — 784 whenever conv 2 or conv 3 is included, 128 (bottleneck width) for any set starting at conv 4 or later, 0 for conv 1 alone — identical across patterns and three seeds (nesting law by construction of the set). The pre-registered "skipping the first adapted layer worsens conditioning" is REFUTED: skipping conv 2 for conv 3 improves `cond_at_1e10` by 3–7 orders (1e2 vs 5e5–1e9); conditioning is set by which module pins (rows × positions vs unknowns), not by shallowness.** | `mnist_conv_bottleneck.pth`, r = 256, N = 8, zero drift, k ∈ {128, 384, 784}, first ∈ {1, 3, 5}, 12 patterns, 3 seeds | 441 rows | r = 64/128 (the discriminating regime) with patterns; drift; a solve; the MLP patterns (366146) | 366147 | read-rows (coordinator, single read) | INTERIM |
| R9 | **When the adapted layer's input moves, the TRUNCATED certificate recovers where the full one cannot: exactness is worth less than rank.** The full certificate keeps its exactness only while `rank B_T = N'` and loses rank as the training span grows (`rank C = r − rank B_T` → 0); at `rank C ≈ k` it returns EXACT zeros at the wrong images (objective 1e-27, image error 0.34) — non-identifiability, not search failure. The truncated certificate keeps rank `r − N` and pays `rho_trunc ≈ K_l·eps_perp` with `K_l` = 0.006–0.031, flat over three decades of drift and all lower-layer ranks; in the same cell it lands 132/200 starts, 7/8 images exactly and 8/8 top-1 by SSIM and features. **CORRECTED 2026-09-18 by pooling three seeds (366211/366216/366221, 135 cells, all three seeds complete):** the RANK rule is sharp at `rank C = k` — **pooled over targets 2 and 4, 265 cells: 141/141 with `rank C ≥ k` recover (min 5 of 8), 120/120 with `rank C ≤ k − 2` recover nothing, and of the 4 cells exactly at `rank C = k − 1` one recovers a single image** (a 1-D solution family passing 9.1e-3 from the truth, inside the 1e-2 bar, 55 of its starts flagged alias) — the boundary behaving like a boundary, not a counterexample. An earlier "exceptionless" reading was from targets 2 only. The ERROR rule is only a monotone trend: median images found 8 / 5 / 1 / 0 over `rho_trunc` bands <1e-4 / 1e-4–7e-4 / 7e-4–2e-3 / >2e-3, but **11 cells above 2e-3 still recover ≥1 image and one recovers 7**, so the single-cell claim "none beyond ~1.5e-3" is WITHDRAWN; the threshold moves ~5× between seeds. `K_l` is flat in the MEDIAN (0.0152–0.0174 over four decades of drift) with a spread of 0.0035–0.534, so it describes the typical cell and cannot convert a drift measurement into a recovery prediction for a given release. One cell with `rank B_T = N'` at 6 % drift gives `rho_full` = 2.9e-15 (excitation ⇒ exactness at arbitrary drift, confirmed on a real net) yet recovers 0/8 because `rank C = 24 < k`. | `mnist_mlp_d15w1000_full` (gate PASS), target layer 2 with layer 1 also adapted, r_target = 64, r_lower ∈ {4,16,64}, N = 8, k = 32, on-chart privates, 200 starts, seed 3 | zero-drift controls at the FP64 floor, 8/8, every arm | 14 of 15 cells still running; one target layer / seed / chart width; momentum + weight decay not yet run; **per-image `verdict` is overwritten by the cell-level `contaminated` flag (labels only; counts correct)** | 366221 (cell), 366163/366203 (smokes) | read-rows (coordinator, single read) | INTERIM |
| R10 | **The two certificates fail for categorically different reasons, visible once the cell-level flag stops overwriting the per-image verdict** (repaired read-side by `relabel_drift_verdicts.py`; 35 of 60 rows change). Over the cell: recovered 540, alias 229, optimum-moved-off-the-truth 187, genuine optimisation failure 4. Under drift the FULL arm is 8/8 `alias (residual zero, wrong image)` in every cell at `T ≥ 20, lr ≥ 0.01` — it reaches a true zero and cannot separate the private image. The TRUNCATED arm is 8/8 `chart-limited`, which with ON-CHART privates cannot mean the chart is the limit: it means the drifted objective's minimiser has moved off the truth. Bias, not a tie. | same cell as R9 | the harness rows are unmodified; the repair is a read-side recomputation with the harness's own rule minus the override | whether the pattern holds in the other 14 cells; the label `chart-limited` is inherited from the ladder harness and is a misnomer in this setting | 366221 | read-rows (coordinator) | INTERIM |
| R11 | **On the deep MLP, WHERE the adapted set starts sets both rank and conditioning, skipping layers COSTS rank (unlike the CNN), and a non-contiguous set produces the gapped law-separating cell contiguous blocks never do.** Single layer at k = 784: layer 1 → 100 ranks at cond 2, layer 8 → 77 at cond 1e8 (no gap), layers 14+15 → ~31 at cond 5e9. Alternating saturates at 424 however many layers are added; the contiguous prefix reaches 624. Random sets follow the nesting accounting (`[1,11]` → 133 = 33 + 100; `[9,11]` → 75; `[10,15]` → 60). **The random set `[3,4,10,15]` predicts 260 (corrected) vs 287 (T5.2), has a real gap (6.2e5 at k = 384, 7.7e5 at k = 784) and measures 260 at both widths** — the corrected law exact, T5.2 wrong by 27. Pattern breakdown: contiguous suffix/middle/single patterns disagree in 0 rows of 522, prefix in 18, random 4-layer draws in 32 of 54. | `mnist_mlp_d15w1000_full` (gate PASS), r = 108, N = 8, zero drift, 12 patterns, k ∈ {128, 384, 784}, 3 seeds, 522 rows | **REPLICATED, job 366737**: same set + 5 neighbours × 3 seeds × 2 widths = 36 rows, formulas differ in all 36, 19 have a real gap (1e3–1e6), and **measured == corrected in 19/19, == T5.2 in 0/19** (T5.2 over-predicts by 26–30), spanning 6 distinct layer sets and all 3 seeds | drift; a solve; other r; why seed 3 loses the gap on most sets | 366146 | read-rows (coordinator, single read) | INTERIM |
| R12 | **A softmax-head adapter fails the excitation hypothesis by construction yet recovers in every cell — the "contaminated ⇒ hopeless" pre-registration is REFUTED, and it is refuted where adapters actually go.** `rank B_T ≤ m − 1 = 10` at a head, so 81 of 105 head cells are flagged contaminated; measured `rho_full` median 3.5e-5 / max 4.9e-4 (against 1.8e-13 on the 24 clean cells) — eight orders of residual lost and **zero cells of total failure: 105 of 105 recover between 1 and 8 of 8 images**, because the head keeps `rank C = r − rank B_T` in 54–56 against `k = 32`. Contamination does cost recovery at the margin (the weakest contaminated cell returns 1 image of 8) but never all of it. Contrast the synthetic contamination figures (median 2.3e-4, max 0.27, T4-C2). **`rank B_T < N'` is therefore a poor predictor of failure at a head.** Separately, `rank C ≥ k` is necessary but not sufficient at thin margin: the only two above-wall failures in 94 hidden-layer cells sit at `rank C = 33` vs `k = 32`, both contaminated, both with the solver finding points BELOW the truth's residual (minimiser displaced, not a tie); the truncated arm recovers 6 and 8 there. Necessary direction unbroken: **no cell with `rank C ≤ k − 2` has recovered anything, 161 cells.** | `mnist_mlp_strong` (gate PASS), targets 2 (hidden) and 3 (head), r_target 64, r_lower ∈ {4,16,64}, N = 8, k = 32, on-chart privates, 200 starts, 3 seeds, 135 cells | the mild-contamination reading is measured only at a 10-wide head on one net | why contamination is mild (the relevance-weighting guess is UNTESTED); deeper heads; other m | 366214/366219/366224 (hidden), 366215/366220/366225 (head) | read-rows (coordinator, single read) | INTERIM |
