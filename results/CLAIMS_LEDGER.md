...
| Q7 | **Correction flag on the two-walls fidelity numbers (e1b C7 / `STATUS` two-wall table):** `two_walls.py:58` selects its eight images with `np.random.RandomState(seed)`, the ladder (`ladder_cell.py:139`) with a torch generator at `seed+7`. On the ladder's own motorcycles pixel PCA at k=16 is **0.341**, not C7's 0.2456. Direction of C7 (no width works) unchanged; **its shortfall ratios must be recomputed on the gate's images before they travel.** | both scripts read at source (coordinator, 2026-09-18) | — | that C7 is withdrawn — it is flagged, pending recomputation by lane 6e | — | read-function | **FLAGGED** |
