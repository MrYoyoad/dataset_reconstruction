...
| Does depth add information additively? | **M4** (9,18,20,20 — rank-preserving regime only), **job 688036** (**NOT** 692603); **M6** real-encoder (job **354535**, attested); `STATUS.md` T5.2 section; audit F11–F14 | **T5.2 FALSE as stated.** Corrected law now **numerically CONFIRMED on a real encoder** (M6: measured 418 = corrected 417, T5.2's 756 refuted 1.8×) — **PASS #1, 2nd audit pending; still a candidate as a *proof***. Theory-regime only; at k≤66 the two laws coincide. M4's sweep is rank-preserving and discriminates neither |
